...
# 				 tf.zeros([c_rsize], dtype=tf.float32), name='t_cdo')
	...
	if c_b_learn_hd:
		...
		# t_cdy = tf.clip_by_value(tf.divide(tf.reduce_sum(hamm, axis=1, name='t_cdy'), float(c_key_dim)/2.0), 0.0, 1.0)

		t_val = tf.divide(tf.reduce_sum(tf.abs(tf.subtract(t_y1, t_y2)), axis=1), float(c_key_dim))
		t_val1 = tf.where(v_targets, t_val, tf.zeros_like(t_val))
		# t_val2 = tf.where(	v_targets, tf.zeros_like(t_val),
		# 					tf.subtract(tf.fill(t_val.get_shape(), 0.3), tf.clip_by_value(t_val, 0.0, 0.3)))
		t_val2 = tf.where(	v_targets, tf.zeros_like(t_val), t_val)
		t_sum.append(t_val)
		t_err1.append(tf.reduce_mean(t_val1))
		t_err2.append(tf.reduce_mean(t_val2))
		# t_sum.append(tf.abs(tf.subtract(t_y1, t_y2)))
		t_cdy = tf.divide(tf.reduce_sum(hamm, axis=1, name='t_cdy'), float(c_key_dim)/2.0)
	...
	# t_diff.append(tf.reduce_mean((v_targets - t_cdy) ** 2, name='t_diff'))
	# t_diff.append(tf.reduce_mean(t_val1 + t_val2, name='t_diff'))
	t_diff.append(tf.divide(tf.reduce_mean(t_val1), tf.reduce_mean(t_val2)))
...
for step in range(c_gg_num_learn_steps):
	...
	if step % c_gg_learn_test_every == 0:
		# err =
		print('err like:', sess.run(t_err1))
		print('err unlike:', sess.run(t_err2))
	...
	if step % c_eval_every == 0:
		...
		print('eval_score = ', np.mean(eval_score), 'list:', eval_score)
	# exit()
