...
try:
	with open(fn, 'rb') as fh:
		...
		for irow, row in enumerate(csvr):
			...
			all_diffs = [int(d) for r in scsv for d in r]
			l_diffs.append([all_diffs[2]])
...
for iiter in range(c_num_iters):
	...
	nd_scores = np.array(l_scores)
	mid_score = l_scores[nd_scores.argsort()[c_mid_score]]
	...
	for isel, sel_mat in enumerate(l_sel_mats):
		if random.random() < c_rnd_asex:
			...
			for icmut in range(c_num_change_muts):
				allele = random.randint(0, c_bitvec_size-1)
				# c_change_mut_prob_change_len = 0.3
				# c_change_mut_num_change = 3
	...
	if (iiter % 10) == 0:
		hist_ibits = [0 for _ in range(num_input_bits)]
		best_sel_mat = l_record_sel_mats[0]
		for iobit in range(c_bitvec_size):
			bit_list = best_sel_mat[iobit][0]
			for iibit in bit_list:
				hist_ibits[iibit] += 1

		print([(iibit, hist_count) for iibit, hist_count in enumerate(hist_ibits) if hist_count > 0])
